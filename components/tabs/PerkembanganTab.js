...
export function PerkembanganTab({ 
  ...
  const filteredHistory = (perkembanganTampil || []).filter(item => {
    ...
    if (searchQuery) {
      ...
      const isMatch = (
        ...
        item.siswa?.programs?.nama?.toLowerCase().includes(q) 
      );
  ...
  // === PAGINASI & SORTING (SUPER AKURAT: Tanggal -> Jam Sesi) ===
  const totalPages = Math.ceil(filteredHistory.length / ITEMS_PER_PAGE);
  
  const sortedHistory = [...filteredHistory].sort((a, b) => {
    // 1. Urutkan dari Tanggal Hari Ini dulu (Z-A)
    ...
    // 2. Jika di hari yang sama, urutkan Jam Sesi Siswa (Pagi -> Sore)
        ...
        <div className="glass-card">
          ...
          <div className="table-wrap" style={{ maxHeight: '500px', overflowY: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: 0 }}>
              <thead style={{ position: 'sticky', top: 0, zIndex: 10 }}>
                <tr>
                  <th style={{ background: '#1e293b', borderBottom: '2px solid rgba(255,255,255,0.1)', width: '15%', padding: '12px' }}>Waktu</th>
                  <th style={{ background: '#1e293b', borderBottom: '2px solid rgba(255,255,255,0.1)', width: '25%', padding: '12px' }}>Siswa</th>
                  <th style={{ background: '#1e293b', borderBottom: '2px solid rgba(255,255,255,0.1)', width: '40%', padding: '12px' }}>Materi</th>
                  <th style={{ background: '#1e293b', borderBottom: '2px solid rgba(255,255,255,0.1)', textAlign: 'center', width: '20%', padding: '12px' }}>Aksi</th>
                </tr>
              </thead>
              <tbody>
                {paginatedData.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td>
                      <b style={{ color: '#60a5fa', fontSize: '14px' }}>{item.siswa?.nama || '-'}</b><br />
                      
                      {/* === PENANDA JAM SESI SISWA === */}
                      <span style={{ fontSize: '11px', color: '#facc15', fontWeight: 'bold' }}>
                        ⏰ Sesi: {item.siswa?.jam_mulai || 'Tidak diset'}
                      </span><br />
                      {/* ============================== */}
                      
                      <span style={{ fontSize: '10px', opacity: 0.7 }}>Guru: {item.users?.nama || '-'}</span><br/>
                ...
                {paginatedData.length === 0 && (
                  <tr><td colSpan="4" style={{ textAlign: 'center', padding: '30px' }} className="text-muted">Data perkembangan belum ada.</td></tr>
                )}
              </tbody>
            </table>
          </div>
          ...
          {totalPages > 1 && (
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '20px', borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '15px' }}>
              ...
              <span style={{ fontSize: '13px', fontWeight: 'bold' }}>Halaman {currentPage} dari {totalPages}</span>
