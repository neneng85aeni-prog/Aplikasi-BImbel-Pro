...
export function KasirTab({ branches, selectedBranchId, setSelectedBranchId, siswaOptions, selectedStudent, kasirForm, setKasirForm, studentScanInfo, scanStudentActive, setScanStudentActive, studentScanText, onSelectStudent, onSubmitKasir, onPrintReceiptDesktop, onPrintReceiptAndroid, programs }) {
  return (
    <div className="grid grid-2">
      <div className="glass-card">
        <h2 className="section-title">Transaksi kasir premium</h2>
        <p className="text-muted">Menu kasir sekarang fokus ke pembayaran. Absensi siswa dipisah ke menu <b>Perkembangan &amp; Absensi</b>.</p>
          ...
          <select 
            ...
            onChange={(e) => {
               ...
               setKasirForm({ 
                  ...
                  nominal: selectedProg ? selectedProg.nominal : (selectedStudent?.nominal || '') 
               });
            }}
          >
            <option value="">-- Gunakan Program Bawaan Siswa --</option>
            {programs?.map((item) => (
              <option key={item.id} value={item.id}>{item.nama} - {formatRupiah(item.nominal)}</option>
            ))}
          </select>
        </div>
