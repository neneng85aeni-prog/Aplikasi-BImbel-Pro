...
export function computeOverview({ pembayaran = [], pengeluaran = [], siswa = [], users = [], branches = [], payrollRows = [], targetDate = new Date() }) {
  ...
  // === LOGIKA BARU: FILTER HANYA SISWA AKTIF ===
  // (Jika status kosong/null, kita anggap masih aktif agar data lama tidak hilang)
  const siswaAktif = siswa.filter(s => s.status !== 'nonaktif' && s.status !== 'Nonaktif');
  ...
  return {
    totalSiswa: siswaAktif.length, // <--- Sekarang hanya menghitung yang aktif
    ...
    studentDistribution: studentDistribution // <--- Pie chart sekarang akurat
  }
}
