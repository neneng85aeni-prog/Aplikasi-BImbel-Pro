...
export function exportRows({ exportType, branches, siswa, users, programs, pembayaran, absensiSiswa, absensiKaryawan, perkembangan, payrollRows, dateFrom, dateTo }) {
  const map = {
    ...
    pembayaran: pembayaran.map((item) => ({ tanggal: item.tanggal, siswa: item.siswa?.nama, nominal: item.nominal, status: item.status, metode: item.metode_bayar, cabang: item.siswa?.branch_nama || item.users?.branch_nama || '-' })),
