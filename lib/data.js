...
export async function upsertSiswa(form, id) {
  // PENCEGAHAN ERROR UUID PADA SISWA
  const payload = { 
    ...form,
...
export async function upsertUserViaRpc(form, id) {
  // PENCEGAHAN ERROR UUID DAN NUMBER PADA KARYAWAN
  return supabase.rpc('app_upsert_user', {
    ...
    p_akses: form.akses,
    p_branch_id: form.branch_id || null, // <-- Solusi UUID Error
    p_no_telepon: form.no_telepon || null,
    p_salary_type: form.salary_type,
    p_salary_fixed: Number(form.salary_fixed) || 0, // <-- Solusi Number Error
