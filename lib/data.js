...
export async function fetchAllData() {
  const [
    ...
    // Data master kecil - tetap ambil semua
    supabase
      .from('branches')
      .select('*')
    ...
    // Data transaksi dibatasi agar tidak menghabiskan Egress
    supabase
      .from('siswa')
      .select('*')
      .order('created_at', { ascending: false })
  ...
  const programMap = new Map(
    programs.map((item) => [item.id, item])
  )
  ...
  const pembayaran = (pembayaranRes.data || []).map((item) => ({
    ...
    branch_id:
      (siswaMap.get(item.siswa_id) || {}).branch_id ||
      (userMap.get(item.kasir_id) || {}).branch_id ||
      null,
  }))
  ...
  const absensiSiswa = (absensiSiswaRes.data || []).map((item) => ({
    ...
    guru_handle:
      userMap.get(item.guru_handle_id) || null,
  }))
  ...
  const absensiKaryawan = (absensiKaryawanRes.data || []).map((item) => ({
    ...
    users:
      userMap.get(item.user_id) || null,

    branch_id:
      (userMap.get(item.user_id) || {}).branch_id ||
      null,
  }))
  ...
  const reviews = (reviewRes.data || []).map((item) => ({
    ...
    reviewer:
      userMap.get(item.reviewer_id) || null,
  ...
  const pengeluaran = (pengeluaranRes.data || []).map((item) => ({
    ...
    branches:
      branchMap.get(item.branch_id) || null,
