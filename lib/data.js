...
export async function fetchAllData() {
  const [
    branchesRes, programsRes, usersRes, siswaRes, pembayaranRes,
    absensiSiswaRes, perkembanganRes, absensiKaryawanRes, bonusRes,
    reviewRes, reviewItemsRes, pengeluaranRes, inventoryRes
  ] = await Promise.all([
  ...
  const firstError =
    branchesRes.error || programsRes.error || usersRes.error || siswaRes.error ||
    pembayaranRes.error || absensiSiswaRes.error || perkembanganRes.error ||
    absensiKaryawanRes.error || bonusRes.error || reviewRes.error ||
    reviewItemsRes.error || pengeluaranRes.error || inventoryRes.error
  ...
  return {
    branches, programs, users, siswa, pembayaran, absensiSiswa,
    perkembangan, absensiKaryawan, bonusManual: bonusRes.data || [],
    reviews, pengeluaran, inventory,
  }
}
...
// 1. FUNGSI UPSERT SISWA (SUDAH DIPERBAIKI & TIDAK DOBEL)
...
// 2. FUNGSI UPSERT USER (DITAMBAHKAN KEMBALI)
