...
export async function upsertProgram(form, id) {
  const payload = { ...form }
  if (!payload.id) delete payload.id // Hapus string kosong dari ID (Penyebab error di menu Program)
...
export async function savePerkembangan(payload) {
  ...
  if (existing?.id) {
    return supabase
      .from('perkembangan')
      .update({ guru_id: payload.guru_id, catatan: payload.catatan, tanggal: payload.tanggal })
      .eq('id', existing.id)
  }

  return supabase.from('perkembangan').insert(payload)
}
...
export async function upsertUserViaRpc(form, id) {
  return supabase.rpc('app_upsert_user', {
    ...
    p_akses: form.akses !== undefined ? form.akses : null,
    p_branch_id: form.branch_id || null,
    p_no_telepon: form.no_telepon || null,
    p_salary_type: form.salary_type,
    ...
    p_trial_ends_at: form.trial_ends_at || null,
    p_batas_jam_masuk: form.batas_jam_masuk || null, // <--- TAMBAH INI
    p_batas_jam_pulang: form.batas_jam_pulang || null // <--- TAMBAH INI
  })
}
